        ...
        if current_time - self.last_freeze_check_time >= 1 and current_time >= self.freeze_cooldown_time:
            ...
            for freeze_hsv in freeze_hsvs:
                ...
                for contour in contours:
                    ...
                    self.click_at(cX, cY)
                    self.logger.log(f'Buzluğa tıklama: {cX} {cY}')
...
if __name__ == "__main__":
    ...
    if not windows:
        print("Belirtilen Blum veya Telegram anahtar kelimelerini içeren pencere yok.")
        exit()
    ...
    choice = int(input("Blum botunun açılacağı pencere numarasını girin: ")) - 1
    ...
    while True:
        try:
            target_percentage = input("Yıldız vuruşlarını rastgele hale getirmek için, 0 ile 1 arasında bir değer girin; burada 1, tüm yıldızların sayısını ifade eder. (Değer seçimi birçok faktöre bağlıdır: ekran boyutu, pencere vb.) Yaklaşık 140-150 yıldız toplamak için 0.04 - 0.06 değerini seçtim. Gerekli değerleri kendiniz seçin: ")
            ...
            print("Geçersiz format. Lütfen bir sayı girin.")

    while True:
        try:
            collect_freeze = int(input("Buzlukla mı tıklayacaksınız? 1 EVET | 2 HAYIR: "))
    ...
    logger.log("Blum için Otomatik Tıklamaya hoş geldiniz.")
    logger.log('Mini oyunu başlattıktan sonra, klavyenizde F6 tuşuna basın.')
    ...
    try:
        ...
        logger.log(f"Bir şeyler yanlış gidiyor:{e}")
    for i in reversed(range(5)):
        print(f"Script, şu durumda sonlanacak:{i}")
        time.sleep(1)
